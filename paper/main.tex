...
guidance. (1)~Under generous budgets on cheap continuous problems,
...
legible: it loses exactly when the landscape couples coordinates.
...
\paragraph{Toward compound algorithms.} The components measured here
compose, and the library itself supplies the parts. As a
proof-of-concept we combined \code{ANAL2} with \code{EXKOR}: spend
100 evaluations on an LP-$\tau$ design, estimate the eigenframe from
\code{ANAL2}'s factor decomposition, and---only when the dominant
eigen-direction explains markedly more influence than the best
original axis---run \code{EXKOR} in the rotated frame. On a rotated
ellipsoid (coordinate search's canonical failure) the compound method
improves the median gap $4.1\times$ over plain \code{EXKOR} at equal
total budget, $1.9\times$ on Zakharov, is neutral on Rosenbrock's
curved valley (a linear frame cannot help), and the influence-ratio
guard keeps it from destroying separable structure on Rastrigin
(script and per-instance results in the repository). A second
composition we have not yet built follows the same logic at the
acquisition level: use the $O(\ell n)$ criterion~\eqref{eq:fiap1} to
prescreen thousands of candidates and reserve the $O(\ell^3)$
Gaussian-process posterior for the shortlist---a budget-matched bridge
between the 1989 trade and the modern one.
