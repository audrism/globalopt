...
budgets modern annealing dominates. All code, data, and scripts are
...
The sweep comprises 18{,}240 Python runs and 13{,}440 R runs (480
problem instances per language and budget tier); no run failed except
the 270 intentional SHGO exclusions above $n=5$ --- where its simplicial
...
\caption{Left: Mor\'e--Wild data profiles (fraction of instances solved
to gap $10^{-2}$ versus evaluations$/(n{+}1)$), Python methods. Right:
the overhead/fidelity decomposition of Section~\ref{sec:fidelity} ---
per-evaluation cost of the overhead-dominated \code{MIG2}
($n{=}10$), and \code{BAYES1} runtime before/after the pruning repair.}
\label{fig:data}
\end{figure}
...
budget---nominally ahead of dual annealing (55.4\%), CMA-ES (47.3\%),
and DIRECT-L (47.1\%)---and 39.0\% in R, tied with DIRECT-L and behind
only GenSA (49.4\%). Tightening the tolerance to $10^{-4}$ preserves
...
ranks 9.6 versus 9.5)---the benchmark-scale confirmation of the
