...
matched evaluation budgets, using performance and data profiles. At the larger budget
the coordinate Wiener-model search EXKOR is competitive with the best
modern methods (solving 56\% of Python instances to gap $10^{-2}$,
versus 55\% for dual annealing and 47\% for CMA-ES and DIRECT-L),
though its strength is concentrated on separable landscapes. At tight
...
GP-Bayesian-optimization tier) and 13{,}440 R runs (480 problem
instances per language and budget tier); no run failed except the 270
...
background load and reported as medians of repeated runs. Four of the
sixteen test families (the Shekel and Hartmann variants) also ship as
demonstration objectives with the 1989 library, a potential
selection bias in its favor; the seeded shifts perturb them away from
the shipped coefficients, and the wrapped methods' strengths and
failures are distributed across shipped and non-shipped families alike
(\code{EXKOR} is perfect on Hartmann-6 but blind on Hartmann-3 and
weak on Shekel). One run per instance follows COCO practice; the
paired tests operate on the 480 instances rather than on within-instance
replicates.
