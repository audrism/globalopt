...
C  Constants imach, rmach, dmach
C
C Modified from I1MACH4.FOR. 1992, Audris Mockus.
C  Using system random number generator RANF in ATS
C
      INTEGER FUNCTION I1MACH(I)
C     PORT FOR CRAY Y-MP8/32 COMPUTER  (based on float.h)
      INTEGER*4 IMACH(16)
      DATA IMACH/5,6,7,6,64,8,
     *2,63,70368744177663,2,47,-8189,8190,94,-8189,8190/
...
c     EQUIVALENCE(RMACH(1),IEQ(1))
      DATA rmach/3.6672077e-2465,2.726870339048517e2465,
     *7.10542735760100e-15,7.10542735760100e-15,3.01029996e-1/
...
c     EQUIVALENCE (DMACH(1),IEQ(1))
      DATA dmach/3.6672077e-2465,2.726870339048517e2465,
     *7.10542735760100e-15,7.10542735760100e-15,3.01029996e-1/
      IF (I.LT.1.OR.I.GT.5) GO TO 1
      D1MACH=DMACH(I)
      RETURN
    ...
    2 FORMAT (/2X,41H**E R R O R  I N  D 1 M A C H** VALUE I =,I2,
     C11H INFEASIBLE)
      ...
      FUNCTION ATS(J)
      COMMON /ATT/X(15)
C uniform generator for cray Y-MP
      ATS = RANF ()
C      X1=X(1)+X(15)
C      IF (X1.GT.1.) X1=X1-1.
C      DO 1 I=2,15
C    1 X(I-1)=X(I)
C      X(15)=X1
C      ATS=X1
